...
(select ws.skill_id from worker_skills ws where ws.worker_id = workers.id) )', id).order('workers.salary asc')
  end

  def search_workers_partial
    workers_full_and_partial = Worker.where('workers.id in (select ws.worker_id from worker_skills ws inner join vacancy_skills vs on ws.skill_id=vs.skill_id where vs.vacancy_id=?)', id).order('workers.salary asc')
